 ...
 * Variant of -m 13100 where the input candidate is a raw NT hash (32 hex -> 16 bytes).
 */
...
DECLSPEC int decrypt_and_check (LOCAL_AS u32 *S, PRIVATE_AS u32 *data, GLOBAL_AS const u32 *edata2, const u32 edata2_len, PRIVATE_AS const u32 *K2, PRIVATE_AS const u32 *checksum, const u64 lid)
{
  ...
  // Header sanity for TGS-REP encrypted ticket part
  ...
  // HMAC over decrypted edata2 with K2
  u32 w0[4], w1[4], w2[4], w3[4];

  w0[0]=K2[0]; w0[1]=K2[1]; w0[2]=K2[2]; w0[3]=K2[3];
...
/* Key schedule: for TGS-REP in hashcat’s 13100, usage constant is 2 (see original kernel) */
DECLSPEC void kerb_prepare (PRIVATE_AS const u32 *K, PRIVATE_AS const u32 *checksum, PRIVATE_AS u32 *digest, PRIVATE_AS u32 *K2)
{
  ...
  // message = 4 bytes little-endian "2"
  w0[0]=2; w0[1]=0; w0[2]=0; w0[3]=0;
  w1[0]=w1[1]=w1[2]=w1[3]=0;
  w2[0]=w2[1]=w2[2]=w2[3]=0;
  w3[0]=w3[1]=w3[2]=w3[3]=0;
...
/* mxx: do NOT apply rules to binary key; hex-decode candidate first */
KERNEL_FQ KERNEL_FA void m13150_mxx (KERN_ATTR_RULES_ESALT (krb5tgs_t))
{
...
/* sxx: same as mxx */
KERNEL_FQ KERNEL_FA void m13150_sxx (KERN_ATTR_RULES_ESALT (krb5tgs_t))
{
  const u64 lid = get_local_id (0);
  const u64 gid = get_global_id (0);
  if (gid >= GID_CNT) return;
